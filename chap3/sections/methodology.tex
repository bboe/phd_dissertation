...
\sprogram{} when explicitly directed to do so through a \emph{save} action. In
order to possess snapshots of students' in-progress \sprogram{s}, we modified
...
student's program have occurred since the last snapshot. We consider clicking
the \emph{green flag} to be a unit of work, because that is how students test
their program. Snapshots are also created whenever the student explicitly
invokes the \emph{save} action. All snapshots for a student are labeled by time
and combined in one zip archive to reduce both the size and number of files we
needed to collect.
