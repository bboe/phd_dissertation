...
To answer these questions we (\localref{sub:by_class})~look at the completion
rate of students by class, (\localref{sub:snapshots})~compare the difficulty of
\sone{} and \stwo{} based on the number of snapshots to completion,
(\localref{sub:approach})~analyze the approach students used in solving the
assignment, (\localref{sub:race})~quantify the number of students who may have
experienced a race condition in Scratch, and (\localref{sub:dce})~quantify the
number of students who may have utilized the \dce{} approach when initially
...
negative progress towards the goal. Also of note is that of the 102 students
who \caught{} at least two sprites, only twelve did not \catch{} the final
sprite.
...
absolute with respect to the \stage{} meaning \emph{up} always orients towards
...
towards the right of the \stage{}, a \emph{turn clockwise 90 degrees} block
will result in the \net{} being oriented towards the bottom of the \stage{}; in
this case, the \abs{} orientation block \pointDIR[down]{} would have the same
effect.
...
labeled \exe{s} by programming them in Scratch, and executing the program up to
twenty times. If within these twenty executions we observed inconsistency in
the \catch{ing} of the \zebra{} the \exe{} was labeled as exhibiting the race
...
\subsection{Snapshots Exhibiting the ``Double Click to Execute'' Behavior}
\locallabel{sub:dce}
